describe('Login', () => {
  let testCode: string
  let adminEmail: string

  beforeEach(() => {
    cy.fixture('userData').then((data) => {
      testCode = data.testCode
      adminEmail = data.adminEmail
    })

    cy.clearLocalStorage() // Clear local storage after each test globally
  })

  it('login', () => {
    cy.visit('/')
    cy.url().should('include', '/login', { timeout: 10000 })

    cy.get('[data-cy=email-input]', { timeout: 10000 }).should('be.visible').type(adminEmail)
    cy.get('[data-cy=email-btn]', { timeout: 10000 }).click()
    cy.get('[data-cy=code-input]', { timeout: 10000 }).type(testCode)

    cy.url().should('include', '/admin')
  })

  it('logout', () => {
    cy.visit('/')
    cy.url().should('include', '/login', { timeout: 10000 })

    cy.get('[data-cy=email-input]', { timeout: 10000 }).should('be.visible').type(adminEmail)
    cy.get('[data-cy=email-btn]', { timeout: 10000 }).click()
    cy.get('[data-cy=code-input]', { timeout: 10000 }).type(testCode)

    cy.url().should('include', '/admin')

    cy.get('[data-cy=menu-btn]', { timeout: 10000 }).click()
    cy.get('[data-cy=logout-btn]', { timeout: 10000 }).click()

    cy.url().should('include', '/login')
  })
})
